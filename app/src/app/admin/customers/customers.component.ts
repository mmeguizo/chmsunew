...
export class CustomersComponent implements OnInit {
  ...
  getAllUsers() {
    ...
    this.customer
      .getAllCustomers()
      .pipe(takeUntil(this.getCustomerSubscription))
      .subscribe((data: any) => {
  ...
  updateCustomer(id) {
    const activeModal = this.ngbModal.open(CustomerModalComponent, {
      size: "sm",
      container: "nb-layout",
      windowClass: "min_height",
      backdrop: "static",
    ...
    activeModal.componentInstance.passEntry.subscribe((receivedEntry) => {
      receivedEntry.success
        ? [
            this.auth.makeToast(
              "success",
              `Updating ${receivedEntry.data?.name}`,
        ...
        : [
            this.auth.makeToast(
              "danger",
              `Updating ${receivedEntry.data?.name || "failed"}`,
              receivedEntry.message
            ),
            this.rerender(),
          ];
    ...
    activeModal.componentInstance.passEntry.subscribe((receivedEntry) => {
      receivedEntry.success
        ? [
            this.auth.makeToast(
              "success",
              `Deleting ${receivedEntry.data?.name || customer.name}`,
              receivedEntry.message
            ),
            this.rerender(),
          ]
    ...
    activeModal.componentInstance.passEntry.subscribe((receivedEntry) => {
      receivedEntry.success
        ...
        : [
            this.auth.makeToast(
              "danger",
              `Adding Customer ${receivedEntry.data?.username || "failed"}`,
              receivedEntry.message
            ),
            this.rerender(),
          ];
  ...
  viewAttachment(id) {
    const activeModal = this.ngbModal.open(FilesComponent, {
      size: "xl",
      container: "nb-layout",
      windowClass: "min_height",
    });
